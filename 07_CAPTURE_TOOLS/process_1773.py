...
def main():
    ...
    counts = {"verified": 0, "needs_human_choice": 0, "added": 0,
              "not_found": 0, "error": 0, "headword_mismatch": 0}
    ...
    for r in results:
        ...
        if wl in index:
            ...
            # SAFEGUARD 1 -- headword mismatch detection. Compare the word we
            # searched for against the headword the site actually returned, so a
            # wrong-entry capture (coin -> Coigne, as -> Ask, let -> -let) is
            # flagged for a human instead of silently entering the library.
            matching_rows = [row for row in rows
                             if headword_matches(w, row.get("headword_label", ""))]
            dropped_rows = [row for row in rows
                            if not headword_matches(w, row.get("headword_label", ""))]

            if not matching_rows:
                # No returned headword matches the searched word: do NOT add it.
                ...
                # At least one row matches: proceed as before, but only using the
                # matching row(s). Recompute the verbatim fields from those rows.
                m_orig = combined(matching_rows)
                m_norm = normalize_text(m_orig)
                m_pos = "; ".join(dict.fromkeys(
                    e["part_of_speech"] for e in matching_rows if e["part_of_speech"]))
                ...
                if dropped_rows:
                    dropped_labels = ", ".join(
                        f"{row.get('headword_label','')!r} ({row.get('title','')})"
                        for row in dropped_rows)
                    note += (f" Headword-mismatch safeguard dropped {len(dropped_rows)} "
                             f"non-matching row(s): {dropped_labels}.")
                new_entry = {
                    "word": w,
                    "source": "Johnson",
                    "year": 1773,
    ...
    lines.append(f"  HEADWORD MISMATCHES (needing manual lookup): {counts['headword_mismatch']}")
    if counts["error"]:
    ...
    if flagged_words:
        ...
        lines.append("")
    lines.append(f"HEADWORD MISMATCHES (needing manual lookup) ({len(mismatch_words)}):")
    lines.append("  The site returned an entry whose headword did NOT match the word")
    lines.append("  searched for (e.g. coin -> Coigne, as -> Ask). These were NOT added")
    lines.append("  to the library and need a manual lookup -- listed separately here so")
    lines.append("  they are not mistaken for verified words or for genuine not-found words.")
    if mismatch_words:
        for i in range(0, len(sorted(mismatch_words)), 10):
            lines.append("  " + ", ".join(sorted(mismatch_words)[i:i+10]))
    else:
        lines.append("  (none)")
    lines.append("")
