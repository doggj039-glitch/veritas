    ...
    for idx, word in enumerate(todo, 1):
        try:
            ...
            if entries:
                ...
                # SAFEGUARD 2 -- automatic root-word retry on NOT_FOUND. Many
                # misses are just inflected forms Johnson only defines under the
                # root (governments -> govern, abridged -> abridge, chusing ->
                # chuse). Try plausible roots in order, one real request each
                # (respecting the same 1.5s courtesy delay), and stop at the first
                # that returns entries AND passes the Safeguard-1 headword check.
                rec = None
                for root in candidate_roots(word):
                    time.sleep(DELAY_BETWEEN_WORDS)  # still a real request to the site
                    root_entries = lookup_word_1773(page, root)
                    if root_entries and any(
                        headword_matches(root, row.get("headword_label", ""))
