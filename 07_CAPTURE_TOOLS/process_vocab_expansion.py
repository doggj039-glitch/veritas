"""
Process the VOCABULARY-EXPANSION pass (2,000 self-grounding-gap words) into the
library.

Loads its base from the CURRENT library snapshot
(VERITAS_definitions_library.BACKUP-BEFORE-VOCAB-EXPANSION.json = the 1,340-entry
state right before this pass), so everything already in the library is preserved
and the 2,000 new words are added on top. Same lineage rule as the missed-words
pass. Reuses the exact verify / normalize / mismatch-check logic.

Reads:  03_LIBRARIES/vocab_expansion_results.json
Base:   03_LIBRARIES/VERITAS_definitions_library.BACKUP-BEFORE-VOCAB-EXPANSION.json
Writes: all FOUR active VERITAS_definitions_library.json copies (identically)
        03_LIBRARIES/vocab_expansion_results.json  (enriched with disposition)
        03_LIBRARIES/vocab_expansion_report.txt / .json
"""
import json, hashlib
...
RESULTS = LIBDIR / "vocab_expansion_results.json"
BASE = LIBDIR / "VERITAS_definitions_library.BACKUP-BEFORE-VOCAB-EXPANSION.json"
...
def main():
    ...
    for r in results:
        ...
        if wl in index:
            ...
            e["retrieved_1773"] = {
                ...
                "note": "Captured verbatim from johnsonsdictionaryonline.com (1773, 4th folio ed.); vocab-expansion pass.",
            }
        ...
        note = (f"Captured verbatim from johnsonsdictionaryonline.com (1773, 4th folio ed.); "
                f"{n} sense-entr{'y' if n==1 else 'ies'}. New word from the vocabulary-expansion "
    ...
    if isinstance(lib["notes"], list):
        lib["notes"].append(
            "Vocabulary-expansion (self-grounding gaps) pass: "
    ...
    (LIBDIR / "vocab_expansion_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=1), encoding="utf-8")

    L = []
    L.append("VERITAS -- Vocabulary-expansion (self-grounding gaps) pass")
    L.append("=" * 60)
    ...
    if not_found_words:
        ...
        L.append("  (none)")
    L.append("")
    (LIBDIR / "vocab_expansion_report.txt").write_text("\n".join(L), encoding="utf-8")
    print("\n".join(L))
