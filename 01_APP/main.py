...
"""
VERITAS
Constitutional Research Engine — Founded on Johnson's Dictionary (1755).
"""
...
# ─── Color Scheme ──────────────────────────────────────────────────────────────
COLORS = {
    "bg_dark":        "#1a1a1a",   # main background — deep charcoal
    "bg_medium":      "#212121",   # panels
    "bg_light":       "#2a2a2a",   # cards, input fields
    "accent":         "#c9a84c",   # gold — primary accent, citations, buttons
    "accent2":        "#8b6914",   # darker gold for hover
    "text_primary":   "#f0f0f0",   # main readable white
    "text_secondary": "#a0a0a0",   # muted secondary text
    "text_highlight": "#ffffff",   # pure white for headings
    "success":        "#4a9e6b",   # green — verified, found
    "warning":        "#c9a84c",   # gold — gaps, warnings
    "danger":         "#c0503a",   # red — errors, conflicts
    "info":           "#5b8fc9",   # blue — citations, links
    "panel_bg":       "#1e1e1e",   # sidebar
    "card_bg":        "#252525",   # research output cards
    "input_bg":       "#2a2a2a",   # text input areas
    "green_light":    "#4a9e6b",
    "orange_light":   "#c9a84c",
    "red_light":      "#c0503a",
}
...
LOGO_ICON_PATH = os.path.join(BASE_DIR, "assets", "veritas_logo_icon.png")
LOGO_BG_PATH = os.path.join(BASE_DIR, "assets", "veritas_logo.png")
...
def _about_dialog():
    messagebox.showinfo("About",
        ...
        f"Constitutional Research Engine\n"
        f"Founded on Johnson's Dictionary (1755).\n\n"
...
class LegalAnalyzerApp:
    ...
    def __init__(self, root):
        ...
        tk.Label(disclaimer_frame, text="VERITAS — CONSTITUTIONAL RESEARCH ENGINE — NOT LEGAL ADVICE — VERIFY ALL RESULTS", 
                 bg="#1a1a1a", fg="#a0a0a0", font=("Segoe UI", 9), pady=4).pack()
    ...
    def _build_styles(self):
        ...
        self.style.configure("Accent.TButton", background=COLORS["accent"], foreground="#1a1a1a", font=("Segoe UI", 11, "bold"))
    ...
    def _build_left_panel(self, parent):
        ...
        ttk.Label(title_frame, text="VERITAS", style="Header.TLabel", foreground=COLORS["accent"]).pack(anchor=tk.W)
        ttk.Label(
            title_frame,
            text=f"Constitutional Research Engine v{APP_VERSION}",
            foreground=COLORS["text_secondary"],
        ).pack(anchor=tk.W)
        ...
        ttk.Button(analysis_frame, text="🔬 Run Research",
                    command=self._run_research, style="Accent.TButton").pack(fill=tk.X, pady=2)
    ...
    def _build_right_panel(self, parent):
        ...
        self.notebook.add(self.research_tab, text="  🔬 VERITAS  ")
        ...
        # Default to Research Map tab on launch
        self.root.after(100, lambda: self.notebook.select(self.research_tab))
        self._sync_view_nav()
    ...
    def _build_terminology_tab(self, parent):
        ...
        self.term_detail_text = scrolledtext.ScrolledText(
            parent, wrap=tk.WORD, font=("Consolas", 12),
            bg=COLORS["input_bg"], fg=COLORS["text_primary"],
    ...
    def _build_ai_tab(self, parent):
        ...
        ai_entry = ttk.Entry(ask_frame, textvariable=self.ai_ask_var, font=("Segoe UI", 11))
    ...
    def _save_last_document_state(self):
        # Do not persist raw uploaded content or metadata to disk.
        try:
    ...
    def _build_research_tab(self, parent):
        ...
        q_frame = ttk.LabelFrame(parent, text="Ask VERITAS", padding=12)
        q_frame.pack(fill=tk.X, padx=8, pady=6)

        self.research_q_var = tk.StringVar()
        q_entry = ttk.Entry(q_frame, textvariable=self.research_q_var, font=("Segoe UI", 13))
        ...
        ttk.Button(btn_row, text="📄 Export Text",
                   command=self._export_research_text).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_row, text="Clear",
                   command=self._clear_research).pack(side=tk.RIGHT, padx=2)

        self.research_status_var = tk.StringVar(
            value="Ready — ask anything about the Constitution or load a document")
    ...
    def _display_research_result(self, result: dict):
        ...
        out.tag_configure("h1",    font=("Segoe UI", 15, "bold"), foreground=COLORS["accent"])
        out.tag_configure("h2",    font=("Segoe UI", 12, "bold"), foreground=COLORS["text_highlight"])
        out.tag_configure("label", font=("Segoe UI", 11, "bold"), foreground=COLORS["info"])
        out.tag_configure("dim",   foreground=COLORS["text_secondary"])
        out.tag_configure("warn",  foreground=COLORS["warning"])
        out.tag_configure("sep",   foreground=COLORS["accent2"])
        ...
        # VERITAS drift timeline (VERITAS_DESIGN.md): founding-era baseline shown;
        # later authorities (the drift) revealed on demand via a clickable link.
        try:
            from veritas_definitions import get_timeline, has_drift
        except Exception:
            get_timeline = lambda _w: []
            has_drift = lambda _w: False
        ...
        if defs:
            ...
            for term, d in defs.items():
                ...
                w(f"    Historical Context: {d.get('historical_context') or '—'}\n", "dim")
                if has_drift(term):
                    tl = get_timeline(term)
                    dyears = [s.get("year") for s in tl if s.get("tier") == "drift" and s.get("year")]
                    ndrift = len(dyears)
                    span = f"{min(dyears)}–{max(dyears)}" if dyears else ""
                    link_tag = f"drift::{term}"
                    out.tag_configure(link_tag, foreground=COLORS["info"], underline=True)
                    w(f"    ⊕ Define the Drift  ({ndrift} later source{'' if ndrift == 1 else 's'}"
                      f"{', ' + span if span else ''})\n", link_tag)
                    out.tag_bind(link_tag, "<Button-1>", lambda _e, t=term: self._open_drift_window(t))
                    out.tag_bind(link_tag, "<Enter>", lambda _e: out.config(cursor="hand2"))
                    out.tag_bind(link_tag, "<Leave>", lambda _e: out.config(cursor=""))
                w("\n")
    ...
    def _open_drift_window(self, term):
        """'Define the Drift' — a Toplevel showing a word's full timeline: the
        founding-era baseline (Johnson 1773, etymology, historical usage) followed
        by each later authority in publication order (Webster, Black's, …), so the
        reader can see how the meaning drifted. See VERITAS_DESIGN.md."""
        try:
            from veritas_definitions import get_timeline
        except Exception:
            return
        tl = get_timeline(term)
        if not tl:
        ...
        win.title(f"Define the Drift — {term}")
        win.configure(bg=COLORS["bg_dark"])
        win.geometry("780x640")
        win.transient(self.root)

        txt = scrolledtext.ScrolledText(
            win, wrap=tk.WORD, bg=COLORS["card_bg"], fg=COLORS["text_primary"],
            insertbackground=COLORS["text_primary"], font=("Segoe UI", 11),
            padx=16, pady=14, borderwidth=0, relief=tk.FLAT)
        txt.pack(fill=tk.BOTH, expand=True)
        txt.tag_configure("h1",    font=("Segoe UI", 16, "bold"), foreground=COLORS["accent"])
        txt.tag_configure("intro", font=("Segoe UI", 9, "italic"), foreground=COLORS["text_secondary"])
        txt.tag_configure("base",  font=("Segoe UI", 12, "bold"), foreground=COLORS["success"])
        txt.tag_configure("drift", font=("Segoe UI", 12, "bold"), foreground=COLORS["info"])
        txt.tag_configure("body",  foreground=COLORS["text_primary"])
        txt.tag_configure("meta",  font=("Segoe UI", 9, "italic"), foreground=COLORS["text_secondary"])

        def w(t, tag=None):
            txt.insert(tk.END, t, tag) if tag else txt.insert(tk.END, t)

        ROLE = {"johnson_1773": "Johnson's Dictionary", "etymology": "Etymology",
                "historical_usage": "Founding-era usage"}
        w(f"{term}\n", "h1")
        w("Founding-era baseline shown first; later authorities show how the meaning drifted.\n\n", "intro")

        for s in tl:
            role = s.get("role", "")
            label = ROLE.get(role, s.get("source", "") or "Source")
            yr, reg = s.get("year", ""), s.get("register", "")
            header = label + (f" — {yr}" if yr else "") + (f"   [{reg}]" if reg else "")
            is_base = s.get("tier") == "baseline"
            w(("● " if is_base else "→ ") + header + ("   (baseline)" if is_base else "   (drift)") + "\n",
              "base" if is_base else "drift")
            if role == "historical_usage":
                ...
                if quotes:
                    ...
                    w("    (no founding-era quotations on file)\n", "meta")
            else:
                body = (s.get("verbatim_text", "") or "").strip()
                w(("    " + body if body else "    (pending — no text yet)") + "\n", "body")
            if s.get("provenance"):
                w("    " + s["provenance"] + "\n", "meta")
            w("\n")
    ...
    def _export_research_text(self):
        """Export the current research result as a plain-text report."""
        if not self.research_result:
            messagebox.showwarning("No Research", "Run a research query first.")
            return
        path = filedialog.asksaveasfilename(
            title="Export Research Text Report",
            defaultextension=".txt",
            filetypes=[("Text", "*.txt"), ("All", "*.*")],
            initialfile=f"research_report_{datetime.now().strftime('%Y%m%d_%H%M')}.txt"
        )
        if not path:
            return
        try:
            from report_generator import ReportGenerator
            rg = ReportGenerator()
            rg.generate_research_text(self.research_result, path)
            messagebox.showinfo("Report Saved", f"Text report saved to:\n{path}")
            self.status_var.set(f"Research text report exported: {path}")
        except Exception as e:
