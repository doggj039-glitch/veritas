...
class PipelineRunner:
    ...
    def save_report(self, result: dict, reports_dir: str = None) -> str:
        ...
        Creates:
            ...
            reports/<timestamp>/report.html   (via ReportGenerator)
            reports/<timestamp>/report.txt    (via ReportGenerator)
        ...
        folder = Path(reports_dir) / f"{ts}_{q_slug}"
        ...
        # report.html and report.txt via ReportGenerator
        try:
            from report_generator import ReportGenerator
            rg = ReportGenerator()
            rg.generate_research_html(result, str(folder / "report.html"))
            rg.generate_research_text(result, str(folder / "report.txt"))
        except Exception as e:
            # Fallback: write minimal HTML so the folder is still usable
            (folder / "report.html").write_text(
                self._render_html(result), encoding="utf-8"
            )
            (folder / "report.txt").write_text(
                f"VERITAS Research Map\n{result.get('question','')}\n"
                f"Error generating full report: {e}",
                encoding="utf-8"
            )

        # manifest.json — phone contract validation + checksums
        try:
            from phone_contract import PhoneContract
            PhoneContract().write_manifest(str(folder))
        except Exception:
            pass  # manifest is additive — never block a save
    ...
    def _identify_terms(self, text: str, errors: list) -> list:
        ...
        try:
            ...
            if hasattr(dp, "find_constitutional_terms"):
                terms = dp.find_constitutional_terms(text)
                if isinstance(terms, list) and terms:
                    return [str(t) for t in terms]

            if hasattr(dp, "find_key_legal_terms"):
                raw = dp.find_key_legal_terms(text)
    ...
    def _define_terms(self, terms: list, errors: list) -> dict:
        ...
        # Curated/verified founding-era store shared with the Blackletter gate
        # (Johnson 1755/1773 + Bailey 1721). Preferred over the app's local
        # literal_dictionary for plain-English meaning.
        try:
        ...
        for term in terms:
            ...
            # Plain-English: VERITAS definitions library first (the sourced,
            # verified founding-era store), then the local literal dictionary as
            # a fallback for any word only present in the older set. Exact
            # headword lookup only -- substring matching would mis-hit
            # (e.g. "jury" inside "injury", "arm" inside "arms").
            v_entry = (VERITAS_DEFINITIONS.get(term.lower())
                       or VERITAS_DEFINITIONS.get(term_lower))
            if v_entry:
                plain_def = v_entry.get("definition", "")
                plain_source = v_entry.get("source", "VERITAS Definitions Library")
            if not plain_def:
            ...
            # Historical context: drafting intent, ratification debate, early legal synthesis.
            # This is NOT a definition -- it is evidence of period usage, kept separate from
            # the plain-English and doctrinal definitions above.
            for key, val in HISTORICAL_CONTEXT_LIBRARY.items():
                if key.lower() == term_lower or term.lower() in key.lower():
                    if isinstance(val, dict):
                        historical_note = val.get("usage_note", "")
                        historical_source = val.get("source", "Historical Context Library")
                    break
            ...
            definitions[term] = {
                ...
                "historical_context_source":  historical_source,
            }
    ...
    def _search_corpus(
        ...
        # Build search query: strip common function words so a full typed
        # question searches on its meaningful words instead of requiring
        # every word (including "does", "the", "on") to appear together.
        _STOPWORDS = {
            ...
            "up","down","out","off","over","under","again","further","once","about","against",
            "between","through","during","before","after","above","below","both","each","few",
        ...
        query_words = []
        seen_w = set()
        for w in _content_words(question) + [w for t in terms[:5] for w in _content_words(t)]:
            if w not in seen_w:
                seen_w.add(w)
                query_words.append(w)
        query = " OR ".join(query_words) if query_words else question.strip()
  ...
  <p>{esc(restatement)}</p>
        ...
        if definitions:
            ...
            for term, defn in definitions.items():
                ...
                hist  = esc(defn.get("historical_context") or "—")
