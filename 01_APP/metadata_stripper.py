...
Uses Pillow for images and Mutagen (optional) for audio/video basic tags.
"""
...
class MetadataStripper:
    ...
    def strip_media_tags(self, input_path):
        """Removes metadata tags from audio/video files using Mutagen (if available)."""
        if not _MUTAGEN_AVAILABLE:
            return False, "Mutagen not installed — audio/video tag stripping unavailable."
        try:
    ...
    def strip_file(self, file_path, output_path=None):
        """Generic entry point — strips metadata based on file extension."""
